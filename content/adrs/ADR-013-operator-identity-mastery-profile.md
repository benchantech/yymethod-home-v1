...
- `adr_image_generation.md` applies the same Why-Not discipline to visual prompt engineering that this set applies to financial planning — compress, constrain, iterate, fix small issues rather than full rewrites
- The ascii_map `[PERSONAL] → [CLIENT] → [EMPLOYER]` is the same entity relationship this conversation documents — expressed as a system diagram instead of a tax structure
- "Employer is inbound only" mirrors the entity transition logic here — clean directional flows, no ambiguity, controlled cash movement
