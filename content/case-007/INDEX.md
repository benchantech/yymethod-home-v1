# ADR Index — AI-Native Venture Launch: Re-Entry Decision, Asset Decomposition, and the Build Resolution
**Method:** YY Method™ Home Edition v2.3
**Captured:** 2026-04-11
**Author:** Case Study Operator
**Freshness Boundary:** C7-001 and C7-002 are permanent structural arguments. C7-003 is active with a 12–18 month reverification trigger. C7-004 is active for the duration of the advisory engagement. C7-005, C7-006, C7-007 are permanent records of the evaluation and its conclusions.

---

## What This Case Is

A structured knowledge artifact capturing the full reasoning chain behind a venture re-entry decision: whether to reacquire a previously owned brand, structure a revenue partnership with the prior acquirer, or build a new operation from scratch using the operator's compounded domain expertise and AI generation capability.

The case traverses three distinct decision layers:

1. **Strategic architecture** — Does the AI capability change the correct operating model, and if so, how?
2. **Engagement structure** — Can a partnership with the prior acquirer serve as a cash flow bridge without transferring the moat that makes the independent venture viable?
3. **Asset evaluation** — Is the distressed brand worth acquiring, and if so, which components of the bundle carry value versus liability?

The reasoning chain includes multiple reversals. The acquisition path appeared viable at price-alone; it was closed by quantitative health metrics. The partnership appeared complex strategically; it was simplified by clearly separating the two knowledge classes. The moat appeared vulnerable to transfer; it was protected by the operational discipline of delivering outputs rather than systems.

This is not a summary. It is a preserved record of judgment exercised under conditions of genuine optionality — where multiple paths remained viable until specific evidence closed them.

---

## The Resolution in One Sentence

The most valuable asset in the bundle had already been extracted before the negotiation began; the remaining components were either disqualified by measurable liabilities or replaceable by the operator's own capability; build fresh.

---

## ADR Hierarchy

```
C7-001  Strategic Architecture — Marketplace Dependency vs. Full-Stack Ownership    [FOUNDATIONAL]
    └── C7-002  AI-Native Design vs. AI-Enhanced Retrofit                           [depends on C7-001]
    └── C7-003  Cash Flow Bridge — Revenue Partnership as Transition Funding         [depends on C7-001]
        └── C7-004  Knowledge Transfer Limits in Advisory Engagement                [depends on C7-002, C7-003]
    └── C7-005  Asset Decomposition — Distressed Brand Re-Engagement                [depends on C7-001]
        └── C7-006  Reputation Debt — Platform Ratings as Non-Recoverable Liability [depends on C7-005]
            └── C7-007  Prior Data as Acquisition-Independent Moat                  [depends on C7-005, C7-006]
C7-008  Publish Reasoning, Protect Deployment — Information Architecture            [CROSS-CUTTING — governs all of C7-001–C7-007]
```

---

## ADR Registry

| ID | Title | Domain | Status | Freshness |
|---|---|---|---|---|
| C7-001 | Strategic Architecture — Marketplace Dependency vs. Full-Stack Ownership | Strategy | Decided | Active — reverify if AI tooling commoditizes across the industry |
| C7-002 | AI-Native Design vs. AI-Enhanced Retrofit | Strategy | Decided | Permanent structural argument |
| C7-003 | Cash Flow Bridge — Revenue Partnership as Transition Funding | Strategy | Decided | Active — reverify at 12–18 month mark |
| C7-004 | Knowledge Transfer Limits in Advisory Engagement | Operations | Decided | Active — for duration of engagement |
| C7-005 | Asset Decomposition — Distressed Brand Re-Engagement | Evaluation | Decided | Permanent — methodology record |
| C7-006 | Reputation Debt — Platform Ratings as Non-Recoverable Liability | Evaluation | Decided — Disqualifying | Permanent — disqualifying finding record |
| C7-007 | Prior Data as Acquisition-Independent Moat | Resolution | Decided | Permanent |
| C7-008 | Publish Reasoning, Protect Deployment — Information Architecture of This Case | Structure | Decided | Permanent |

---

## Cascade Rules

If the AI generation tooling becomes broadly commoditized (domain knowledge encoding advantage disappears) → re-evaluate C7-001 and C7-002; the moat argument weakens significantly  
If the advisory engagement partner independently develops AI generation capability → re-evaluate C7-004; the "outputs not architecture" discipline becomes more urgent, not less  
If the advisory engagement terms shift to gross revenue share from net → renegotiate terms; see C7-003 for the note on cost manipulation risk in net-based structures  
If the partner offers to sell only the design catalog and customer data (without the platform account or brand name) → re-evaluate C7-005 and C7-007; the disqualifying components may be separable  
If the owned channel reaches self-funding before the 12–18 month bridge window → close the advisory engagement ahead of schedule; opportunity cost has turned negative  
If a competitor demonstrates AI-native, niche-targeted production in the same categories → immediately accelerate AEO/GEO content build; the window identified in C7-001 is narrowing  
If the historical conversion data is legally challenged as improperly extracted → retain legal counsel; evaluate C7-007's foundational premise
If any operator adds case 007 content to this repo → C7-008 and ADR-026 jointly govern what may be added; protected elements are enumerated in C7-008; system-wide principle is in ADR-026
