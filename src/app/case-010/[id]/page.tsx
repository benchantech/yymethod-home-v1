...
import { adrsCase010, getADRCase010, domainLabelCase010 } from "@/lib/adrs-case-010";
...
const ADR_FILE_MAP: Record<string, string> = {
  "C10-001": "C10-001-child-as-author.md",
  "C10-002": "C10-002-original-character-requirement.md",
  "C10-003": "C10-003-private-prototype-boundary.md",
  "C10-004": "C10-004-google-oauth-allowlist.md",
  "C10-005": "C10-005-pseudonymous-portfolio-structure.md",
  "C10-006": "C10-006-no-public-indexing.md",
  "C10-007": "C10-007-provenance-process-logging.md",
  "C10-008": "C10-008-parent-built-engine.md",
  "C10-009": "C10-009-adult-consent-lifecycle.md",
  "C10-010": "C10-010-commercialization-gate.md",
};
...
async function getADRRaw(adrId: string): Promise<string | null> {
  ...
  try {
    const filePath = path.join(process.cwd(), "content", "case-010", filename);
...
export function generateStaticParams() {
  return adrsCase010.map((adr) => ({ id: adr.id.toLowerCase() }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const adr = getADRCase010(id.toUpperCase());
  if (!adr) return {};
  const title = `${adr.id}: ${adr.title} — YY Method™ Home Edition`;
  const description = `${adr.summary} Decision status: ${adr.status}. Case 010 — Child-Originated Character Studio.`;
  return {
    title,
    description,
    ...
    openGraph: {
      title,
      description,
      url: `https://home.yymethod.com/case-010/${adr.id}`,
...
export default async function ADRCase010Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const adr = getADRCase010(id.toUpperCase());
  if (!adr) notFound();
  ...
  const [content, rawContent, dependsOnADRs] = await Promise.all([
    ...
    getADRRaw(adr.id),
    Promise.resolve(adr.dependsOn.map((depId) => getADRCase010(depId)).filter(Boolean)),
  ]);

  const faqItems = rawContent ? extractFaqItems(rawContent) : [];
  const dependents = adrsCase010.filter((a) => a.dependsOn.includes(adr.id));
  const currentNum = parseInt(adr.number);
  const prevADR = adrsCase010.find((a) => parseInt(a.number) === currentNum - 1);
  const nextADR = adrsCase010.find((a) => parseInt(a.number) === currentNum + 1);
  ...
  return (
    <>
      ...
      <JsonLd data={breadcrumbSchema([
        ...
        { name: "Case 010", url: "https://home.yymethod.com/case-010" },
        { name: adr.id, url: `https://home.yymethod.com/case-010/${adr.id}` },
      ])} />
    ...
    <main className="min-h-screen p-6 md:p-10 max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>/</span>
        <Link href="/case-010" className="hover:text-foreground">Case #010</Link>
        <span>/</span>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="outline" className="text-xs font-mono">{domainLabelCase010[adr.domain]}</Badge>
            ...
            {dependsOnADRs.length > 0 ? (
              <div className="space-y-1">
                {dependsOnADRs.map((dep) => dep && (
                  <Link key={dep.id} href={`/case-010/${dep.id.toLowerCase()}`}
      ...
      <div className="flex items-center justify-between">
        {prevADR ? (
          <Link href={`/case-010/${prevADR.id.toLowerCase()}`}
            className="text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
            ← {prevADR.id}
          </Link>
        ) : (
          <Link href="/case-010" className="text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
            ← Case 010
          </Link>
        )}
        {nextADR && (
          <Link href={`/case-010/${nextADR.id.toLowerCase()}`}
            className="text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
            {nextADR.id} →
          </Link>
        )}
