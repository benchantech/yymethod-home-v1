...
import { adrsCase003, getADRCase003, domainLabelCase003 } from "@/lib/adrs-case-003";
...
async function getADRRaw(adrId: string): Promise<string | null> {
  ...
  try {
    const filePath = path.join(process.cwd(), "content", "case-003", filename);
...
export function generateStaticParams() {
  return adrsCase003.map((adr) => ({ id: adr.id.toLowerCase() }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const adr = getADRCase003(id.toUpperCase());
  if (!adr) return {};
  const title = `${adr.id}: ${adr.title} — YY Method™ Home Edition`;
  const description = `${adr.summary} Decision status: ${adr.status}. Case 003 — Quest 3, Cognitive Theft & AI Governance.`;
  return {
    title,
    description,
    ...
    openGraph: {
      title,
      description,
      url: `https://home.yymethod.com/case-003/${adr.id}`,
...
export default async function ADRCase003Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const adr = getADRCase003(id.toUpperCase());
  if (!adr) notFound();
  ...
  const [content, rawContent, dependsOnADRs] = await Promise.all([
    ...
    getADRRaw(adr.id),
    Promise.resolve(adr.dependsOn.map((depId) => getADRCase003(depId)).filter(Boolean)),
  ]);

  const faqItems = rawContent ? extractFaqItems(rawContent) : [];
  const dependents = adrsCase003.filter((a) => a.dependsOn.includes(adr.id));
  const currentNum = parseInt(adr.number);
  const prevADR = adrsCase003.find((a) => parseInt(a.number) === currentNum - 1);
  const nextADR = adrsCase003.find((a) => parseInt(a.number) === currentNum + 1);
  ...
  return (
    <>
      ...
      <JsonLd data={breadcrumbSchema([
        ...
        { name: "Case 003", url: "https://home.yymethod.com/case-003" },
        { name: adr.id, url: `https://home.yymethod.com/case-003/${adr.id}` },
      ])} />
    ...
    <main className="min-h-screen p-6 md:p-10 max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>/</span>
        <Link href="/case-003" className="hover:text-foreground">Case #003</Link>
        <span>/</span>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="outline" className="text-xs font-mono">{domainLabelCase003[adr.domain]}</Badge>
            ...
            {dependsOnADRs.length > 0 ? (
              <div className="space-y-1">
                {dependsOnADRs.map((dep) => dep && (
                  <Link key={dep.id} href={`/case-003/${dep.id.toLowerCase()}`}
      ...
      <div className="flex items-center justify-between">
        {prevADR ? (
          <Link href={`/case-003/${prevADR.id.toLowerCase()}`}
            className="text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
            ← {prevADR.id}
          </Link>
        ) : (
          <Link href="/case-003" className="text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
            ← Case 003
          </Link>
        )}
        {nextADR && (
          <Link href={`/case-003/${nextADR.id.toLowerCase()}`}
            className="text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
            {nextADR.id} →
          </Link>
        )}
