...
import { adrsCase013 } from "@/lib/adrs-case-013";
...
export default function sitemap(): MetadataRoute.Sitemap {
  const staticRoutes: MetadataRoute.Sitemap = [
    ...
    { url: `${BASE_URL}/case-013`, lastModified: "2026-07-24", changeFrequency: "monthly", priority: 0.8 },
  ];
  ...
  const case009Routes: MetadataRoute.Sitemap = adrsCase009.map((adr) => ({
    url: `${BASE_URL}/case-009/${adr.id}`,
    lastModified: adr.date,
    changeFrequency: "weekly" as const,
    priority: 0.7,
  }));

  const case010Routes: MetadataRoute.Sitemap = adrsCase010.map((adr) => ({
    url: `${BASE_URL}/case-010/${adr.id}`,
    lastModified: adr.date,
    changeFrequency: "weekly" as const,
    priority: 0.7,
  }));
  ...
  const case013Routes: MetadataRoute.Sitemap = adrsCase013.map((adr) => ({
    url: `${BASE_URL}/case-013/${adr.id}`,
    lastModified: adr.date,
    changeFrequency: "monthly" as const,
    priority: 0.7,
  }));
  ...
  return [...staticRoutes, ...case001Routes, ...case002Routes, ...case003Routes, ...case004Routes, ...case005Routes, ...case006Routes, ...case007Routes, ...case008Routes, ...case009Routes, ...case010Routes, ...case011Routes, ...case012Routes, ...case013Routes];
}
