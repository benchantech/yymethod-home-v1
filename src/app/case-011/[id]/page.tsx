...
import { adrsCase011, getADRCase011, domainLabelCase011 } from "@/lib/adrs-case-011";
import { statusColor } from "@/lib/adrs";

const ADR_FILE_MAP: Record<string, string> = {
  "C11-001": "C11-001-real-yy-first-class-viewpoint.md",
  "C11-002": "C11-002-adult-syntax-childlike-wonder.md",
  "C11-003": "C11-003-adult-directed-family-learning-posture.md",
  "C11-004": "C11-004-gamified-progression-loop.md",
  "C11-005": "C11-005-sandbox-to-graduation-pathway.md",
  "C11-006": "C11-006-benchantech-yy-method-academy.md",
  "C11-007": "C11-007-parallel-metaphor-families-businesses.md",
  "C11-008": "C11-008-client-advisory-funding-boundary.md",
  "C11-009": "C11-009-family-first-domain-boundary.md",
...
async function getADRRaw(adrId: string): Promise<string | null> {
  ...
  try {
    const filePath = path.join(process.cwd(), "content", "case-011", filename);
...
export function generateStaticParams() {
  return adrsCase011.map((adr) => ({ id: adr.id.toLowerCase() }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const adr = getADRCase011(id.toUpperCase());
  if (!adr) return {};
  const title = `${adr.id}: ${adr.title} — YY Method™ Home Edition`;
  const description = `${adr.summary} Decision status: ${adr.status}. Case 011 — YY Method Academy.`;
  return {
    title,
    description,
    ...
    openGraph: {
      title,
      description,
      url: `https://home.yymethod.com/case-011/${adr.id}`,
...
export default async function ADRCase011Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const adr = getADRCase011(id.toUpperCase());
  if (!adr) notFound();

  const content = await getADRContent(adr.id);
  const dependsOnADRs = adr.dependsOn.map((depId) => getADRCase011(depId)).filter(Boolean);
  const dependents = adrsCase011.filter((a) => a.dependsOn.includes(adr.id));
  const currentNum = parseInt(adr.number);
  const prevADR = adrsCase011.find((a) => parseInt(a.number) === currentNum - 1);
  const nextADR = adrsCase011.find((a) => parseInt(a.number) === currentNum + 1);
  ...
  return (
    <main className="min-h-screen p-6 md:p-10 max-w-4xl mx-auto space-y-8">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>/</span>
        <Link href="/case-011" className="hover:text-foreground">Case #011</Link>
        <span>/</span>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="outline" className="text-xs font-mono">{domainLabelCase011[adr.domain]}</Badge>
            ...
            {dependsOnADRs.length > 0 ? (
              <div className="space-y-1">
                {dependsOnADRs.map((dep) => dep && (
                  <Link key={dep.id} href={`/case-011/${dep.id.toLowerCase()}`}
      ...
      <div className="flex items-center justify-between">
        {prevADR ? (
          <Link href={`/case-011/${prevADR.id.toLowerCase()}`}
        ...
        <Link href="/case-011" className="text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
          Case #011 Registry
        </Link>
        {nextADR ? (
          <Link href={`/case-011/${nextADR.id.toLowerCase()}`}
            className="text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
            {nextADR.id} →
          </Link>
        ) : (
