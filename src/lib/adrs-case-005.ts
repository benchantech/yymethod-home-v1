...
export const adrsCase005: ADRCase005[] = [
  {
    id: "C5-001",
    number: "001",
    title: "Initial Idea — Upgrade Home, Rent Current to Trusted Friends",
    status: "Decided",
    freshness: "Active",
    freshnessNote: "Active. Expires if friends are no longer suitable tenants or if the upgrade direction changes materially.",
    date: "2026-04-05",
    domain: "framing",
    position: "Foundational",
    dependsOn: [],
    summary: "Keep the current home for posterity — the option to move back someday — while renting it to trusted friends to prevent it from becoming a cash bleed. Choose friends as tenants specifically for trustworthiness to protect bandwidth. Explore the upgrade (buy or rent, open mode) pulled by community, convenience, and lifestyle. Nothing irreversible yet.",
    tags: ["initial-idea", "anchor-home", "friends-as-tenants", "legacy", "bandwidth-protection", "exploratory"],
  },
  {
    id: "C5-002",
    number: "002",
    ...
    position: "Derived",
    dependsOn: ["C5-001"],
    summary: "Running the numbers revealed a 7-figure opportunity cost. The delta between current and upgraded housing cost is a few hundred thousand dollars. Compounded over several decades at reasonable investment returns, that capital becomes a 7-figure foregone sum. Every dollar spent on the lifestyle upgrade is a dollar not compounding. The upgrade is not free — it has a generational price tag.",
  ...
  },
  {
    ...
    summary: "Renting the destination before buying de-risks both axes simultaneously. Family alignment: test the community before committing capital — exit is clean if it doesn't work. Financing: salary qualifies for a large bank loan but the operator is opposed on principle. Selling the current home destroys the legacy anchor (C5-001). A family loan introduces external obligation. Rent-first sidesteps the funding trilemma entirely. Why-not: a rental move is still a move — the disruption cost is real.",
    tags: ["rent-first", "de-risk", "no-bank-loan", "funding-trilemma", "family-loan-avoided", "test-environment"],
  },
  {
    id: "C5-008",
    number: "008",
